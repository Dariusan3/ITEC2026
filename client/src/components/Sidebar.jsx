    ...
    <div className="w-72 h-full border-l flex flex-col"
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {messages.length === 0 && (
          <p className="text-xs text-center mt-8" style={{ color: 'var(--text-secondary)' }}>
            ...
            <br />They'll appear as blocks in the editor.
          </p>
        )}
      ...
      <div className="p-3 border-t" style={{ borderColor: 'var(--border)' }}>
        ...
        <button
          ...
          className="w-full mt-2 text-xs font-semibold py-1.5 rounded transition-opacity"
          style={{
  ...
  if (msg.role === 'user') {
    return (
      <div className="text-xs p-2 rounded" style={{ background: 'var(--bg-tertiary)', color: 'var(--text-primary)' }}>
        <span className="font-semibold" style={{ color: 'var(--blue)' }}>You: </span>
        {msg.content}
      </div>
    )
  }
  if (msg.role === 'ai') {
    return (
      <div className="text-xs p-2 rounded border" style={{
        ...
        <span className="font-semibold" style={{ color: 'var(--accent)' }}>Claude: </span>
        {msg.content}
        {msg.blockId && (
